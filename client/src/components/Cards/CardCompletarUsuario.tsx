...
const CardCompletarUsuario = ({ name, email, action }: Props): JSX.Element => {
  return (
    <div className='w-full h-[166px] rounded-lg border-2 shadow-md dark:bg-natgas-azul dark:border-natgas-azul text-center'>
     ...
     <button onClick={action} className="rounded-full px-8 py-2 bg-natgas-azul text-white mt-4 dark:bg-natgas-azul-claro"><Link to={`/profile/${email}/complete`}/>Registrar</button>
    </div>
  )
}
